...
export async function resetDatabase(db: Db): Promise<void> {
  await db.execute(
    sql`TRUNCATE TABLE alert, count, routine_check, pending_mismatch_selection, awaiting_ingestion_count, inventory_movement, daily_ingestion_run, processed_sales_file, processed_receipt_file, processed_waste_file, supply, routine, store CASCADE`,
  );
}
